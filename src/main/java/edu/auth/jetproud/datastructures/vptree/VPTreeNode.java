package edu.auth.jetproud.datastructures.vptree;

import edu.auth.jetproud.datastructures.vptree.distance.DistanceFunction;
import edu.auth.jetproud.datastructures.vptree.exceptions.PartitionException;
import edu.auth.jetproud.datastructures.vptree.thresholdselectionstrategy.ThresholdSelectionStrategy;
import edu.auth.jetproud.datastructures.vptree.utils.NearestNeighborCollector;
 ...
 */
class VPTreeNode<P extends Comparable<P>, E extends P> implements Serializable{
    ...
    public VPTreeNode(final Collection<E> points, final DistanceFunction<P> distanceFunction,
        ...
        /// TODO: Check for logic error and maybe revert to old impl
        /*
         ...
         */
        this.points.sort(Comparable::compareTo);
    ...
    protected int minHeight() {
        ...
        else return Math.min(this.closer.minHeight(), this.farther.minHeight()) + 1;
    }
    ...
    protected VPTreeNode get_farther() {
        return this.farther;
    }
    ...
    public void findPartitions(final P queryPoint, final double maxDistance, final Collection<String> collection, final int height, final Boolean active) {
        ...
        // We want to search any of this node's children that intersect with the query region
        if (distanceFromVantagePointToQueryPoint <= this.threshold) {
            if(height == 1) collection.add(this.get_closer().partition + "&" + active);
            else this.closer.findPartitions(queryPoint, maxDistance, collection, height - 1, active);
        }else if (distanceFromVantagePointToQueryPoint <= this.threshold + maxDistance){
            if(height == 1) collection.add(this.get_closer().partition + "&" + false);
            else this.closer.findPartitions(queryPoint, maxDistance, collection, height - 1, false);
        }
        if (distanceFromVantagePointToQueryPoint > this.threshold) {
            if(height == 1) collection.add(this.get_farther().partition + "&" + active);
            else this.farther.findPartitions(queryPoint, maxDistance, collection, height - 1, active);
        }else if (distanceFromVantagePointToQueryPoint > this.threshold - maxDistance){
            if(height == 1) collection.add(this.get_farther().partition + "&" + false);
