package edu.auth.jetproud.datastructures.mtree.split;
...
public interface SplitFunction<DATA> {
    ...
    static <DATA> SplitFunction<DATA> composedOf(final PromotionFunction<DATA> promotionFunction, final PartitionFunction<DATA> partitionFunction) {
        return (data, distanceFunction) -> {
            Pair<DATA> promoted = promotionFunction.process(data, distanceFunction);
            Pair<Set<DATA>> partitions = partitionFunction.process(promoted, data, distanceFunction);
